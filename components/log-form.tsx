...
export function LogFormWithUser({ user }: { user: LogFormUser }) {
  const [catalog, setCatalog] = useState<CurriculumCatalog>([]);
  const [formState, setFormState] = useState(initialState);
  const [submittedMessage, setSubmittedMessage] = useState("");
  const [status, setStatus] = useState("Loading curriculum...");
  ...
  return (
    <form
      ...
      onSubmit={async (event) => {
        ...
        const payload = (await response.json()) as { source?: string; success?: boolean; error?: string };
        if (!response.ok || !payload.success) {
          setSubmittedMessage(payload.error ?? "Unable to save log right now.");
          setSubmitting(false);
          return;
        }
    ...
    >
      ...
      <div className="space-y-6">
        ...
        <div className="glass-panel rounded-[1.9rem] p-6">
          ...
          <div className="mt-5 space-y-4">
            <div className="rounded-[1.4rem] bg-white/70 p-4">
              <p className="text-sm font-medium text-slate-700">Logged In User</p>
              <p className="mt-1 text-lg font-semibold tracking-[-0.03em] text-slate-800">{user.name}</p>
              <p className="mt-1 text-xs text-slate-500">{user.email}</p>
            </div>
            <div className="rounded-[1.4rem] bg-white/70 p-4">
              <p className="text-sm font-medium text-slate-700">Database Path</p>
              <p className="mt-1 text-sm leading-7 text-slate-600">
                Form submissions go to /api/logs and are automatically tagged with your authenticated account identity.
              </p>
            </div>
          </div>
